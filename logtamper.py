...
def getXtmp(filename, username, hostname):
    ...
    try:
        ...
        showMessage(f"Cannot open file: {filename}, caught {e}")
    finally:
        fp.close()
    return xtmp
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="MODE",
        default="1",
        help="1: utmp, 2: wtmp, 3: lastlog [default: 1]",
    )
    ...
    if len(args) < 3:
        if options.MODE == "1":
            ...
            if (
                ...
                or options.HOSTNAME is None
                or options.TTYNAME is None
                or options.DATELINE is None
            ):
                showMessage("+[Warning]: Incorrect parameter.\n")
